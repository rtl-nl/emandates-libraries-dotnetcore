...
namespace eMandates.Merchant.Library.XML.Utils
{
    internal class XmlSignature
    {
        ...
        /// </summary>
        private static Regex[] AlgorithmsRegex_EnvelopedSignature = { 
                                    ...
                                    new Regex("<(([^<>:]*):)?Transform[^\\\">]+\"http://www\\.w3\\.org/2001/10/xml-exc-c14n#\"", RegexOptions.Compiled),
        ...
        public static XmlElement Sign(ref XmlDocument doc, X509Certificate2 certificate, XmlElement signatureContainer, string dsPrefix = "", bool useFingerprint = true)
        {
            ...
            //todo cip!!! why the first element?
        ...
        private static string ReplaceKeyWithFingerprint(string fingerprintHex, string dsPrefix, string xml)
        {
            ...
            {
                string keyNameTag = string.Format("<KeyName>{0}</KeyName>", fingerprintHex.Replace("-", ""));
                xml = System.Text.RegularExpressions.Regex.Replace(xml, "<KeyValue>.*</KeyValue>", keyNameTag);
            }
            else
            {
                string keyNameTag = string.Format("<{1}:KeyName>{0}</{1}:KeyName>", fingerprintHex.Replace("-", ""), dsPrefix);
                xml = System.Text.RegularExpressions.Regex.Replace(xml,
                                                                   string.Format("<{0}:KeyValue>.*</{0}:KeyValue>", dsPrefix),
                                                                   keyNameTag);
            }
            return xml;
        }
        ...
        public static XmlDocument CreateXmlDocumentFromXml(string xml)
        {
